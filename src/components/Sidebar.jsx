...
const Sidebar = () => {
  ...
  return (
    <div className={`${location.pathname == '/' ? `h-[100vh] overflow-y-scroll no-scroll sticky top-0 max-lg:fixed max-lg:top-3 max-lg:mt-12 transition-all ease-in-out duration-500
        ...
        {menuLists.map((menu) => {
          return (
            <div key={menu.id} className='flex items-center gap-5 cursor-pointer '>
              ...
              {sidebar || showSidebar ? <h1 className={`font-semibold max-sm:text-sm`}>{menu.title}</h1> : ''}
            </div>
          )
        })}
        <div className='flex flex-col gap-5'>
          <h1 className={`amx-sm:text-sm  font-semibold text-gray-400 ${sidebar || showSidebar ? 'visible' : 'invisible'}`}>Subscriptions</h1>
          {
            Array(5).fill(5).map((_, i) => {
              return <div key={i} className='flex items-center gap-5 cursor-pointer'>
                ...
                {sidebar || showSidebar ? <h1 className='max-sm:text-sm  font-semibold text-gray-400'>GreatStack</h1> : ''}
              </div>
            })
          }
        </div>
